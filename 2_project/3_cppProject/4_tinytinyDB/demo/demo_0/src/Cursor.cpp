...
Cursor::Cursor(Table* table, uint32_t targetLeafKey)
    : m_table(table) {
...
void Cursor::leaf_node_find(void* rootNode, const uint32_t& targetLeafKey) {
    ...
    uint32_t num_cells = *m_table->leaf_node_num_cells(rootNode);

    uint32_t min = 0;
    uint32_t max = num_cells;
    while (max != min) {
        uint32_t mid = (min + max) / 2;
        uint32_t key_at_index = *m_table->leaf_node_key(rootNode, mid);
...
void Cursor::internal_node_find(void* rootNode, const uint32_t& targetLeafKey) {
    ...
    uint32_t childPageKey = *m_table->internal_node_child(rootNode, child_index);
...
uint32_t Cursor::internal_node_find_child(void* rootNode, const uint32_t& targetLeafKey) {
    uint32_t num_keys = *m_table->internal_node_num_keys(rootNode);

    uint32_t min = 0;
    uint32_t max = num_keys;
    while (max != min) {
        uint32_t mid = (min + max) / 2;
        uint32_t key_at_index = *m_table->internal_node_key(rootNode, mid);
        if (key_at_index >= targetLeafKey) {
...
void* Cursor::curPos() const { return m_table->leaf_node_value(m_curRootNode, m_leaf_idx); };

void Cursor::next() {
    m_leaf_idx++;
    if (m_leaf_idx >= *(m_table->leaf_node_num_cells(m_curRootNode))) {
        uint32_t next_page_num = *m_table->leaf_node_next_leaf(m_curRootNode);
        if (next_page_num == 0) {
            m_endOfTable = true;
        } else {
