...
namespace jc {
    ...
    class DAGGraph {
      ...
      private:
        ...
        std::vector<K> TopologicalSequence(
            const std::set<K>& connected_components,
    ...
    template <typename K, typename V>
    inline void DAGGraph<K, V>::Walk(
        std::function<void(const K& k, const V& v)> f, bool start_from_head) {
        if (sequences_start_from_head_.empty()) {
            RefreshWalkSequences();
        }
        const std::vector<std::vector<K>>& seqs_to_walk =
            start_from_head ? sequences_start_from_head_
                            : sequences_start_from_tail_;
    ...
    inline std::unordered_set<K> DAGGraph<K, V>::NextKeys(const K& key) {
        ...
        for (std::vector<K>& seq : sequences_start_from_head_for_next_) {
            ...
            for (DAGNode<K, V>* v : nodes) {
                ...
                bool no_prev_node_in_seq = std::all_of(
                    std::begin(prev_nodes), std::end(prev_nodes),
                    [&](DAGNode<K, V>* in_node) {
    ...
    inline void DAGGraph<K, V>::RefreshWalkSequences() {
        ...
        const std::vector<std::set<K>> connected_components =
            ConnectedComponents();
    ...
    template <typename K, typename V>
    inline std::vector<std::set<K>> DAGGraph<K, V>::ConnectedComponents()
        const {
...
namespace jc::test {
    ...
    void test() {
        ...
        {
            std::vector<int> v;
            std::vector<int> start_order{13, 6, 7, 8, 11, 12, 9,
                                         10, 0, 1, 3, 2,  4,  5};
            d.Walk(
                [&](int key,
                    const std::unique_ptr<MockPipelineEngine>& pipeline) {
                    pipeline->Start();
                    v.emplace_back(key);
                },
                start_from_head);
        ...
        {
            std::vector<int> v;
            std::vector<int> stop_order{13, 7, 6, 10, 9, 8, 12,
                                        11, 5, 2, 4,  1, 3, 0};
            d.Walk(
                [&](int key,
                    const std::unique_ptr<MockPipelineEngine>& pipeline) {
