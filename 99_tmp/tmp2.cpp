`std::
    memory_order_consume` 是一种内存顺序，它确保数据依赖性对于同一线程的后续读取操作强制排序。为了更好地理解这一点，我们可以通过一个具体的例子来说明。
...
```
        ...
        1. *
        *生产者线程(`producer`) * *： -
    创建一个新的整数指针 `p`，指向值为 42 的整数。 - 将 `data` 设置为 100。 -
    使用 `std::
            memory_order_release` 将指针 `p` 存储到原子变量 `ptr` 中。这一步确保在发布指针之前，所有对 `data` 的写入操作都已经完成。

        2. *
        *消费者线程(`consumer`) * *： -
    不断尝试从原子变量 `ptr` 中加载指针 `p`，直到成功。 -
    使用 `std::
        memory_order_consume` 加载指针 `p`。这确保了所有依赖于 `p` 的操作（如读取 `data` 和解引用 `p`）不会被重排序到加载操作之前。
    -
    打印 `data` 和 `* p` 的值。
