...
linux有众多包管理工具，如debian系列的apt，red hat系列的yum等等。主要是不存在一个强力的标准，各自社区搞各自的。比如说不同社区对包的发布频率，检查标准不一致，互相扯皮，或者有小功能不支持，但用的包管理工具是另一个社区的，对面没需求，迟迟不实现，自己恼火了就自己写了。但本质上包管理工具都是一样的，配置一个源，即从哪里下载包；解决依赖问题；提供交互能力。
