...
本文我们就继续深入聊聊渲染流水线中的 CSS。因为 CSS 是页面中非常重要的资源，它决定了页面最终显示出来的效果，并影响着用户对整个网站的第一体验。所以，搞清楚浏览器中的 CSS 是怎么工作的很有必要，只有理解了 CSS 是如何工作的，你才能更加深刻地理解如何去优化页面。
...
这个 CSSOM 体现在 DOM 中就是 document.styleSheets。具体结构你可以去查阅相关资料，这里我就不过多介绍了，你知道 CSSOM 的两个作用是怎样的就行了。
...
#### 1. 什么是CSSOM？CSSOM有什么用？
CSSOM（CSS Object Model）是一种将CSS表示为JavaScript对象的方式，使得开发者可以通过JavaScript来操作CSS。它是浏览器解析CSS并创建渲染树的一部分。

以下是CSSOM的一些主要用途：

1. **动态更改样式**：通过CSSOM，开发者可以使用JavaScript动态地更改页面的样式。例如，你可以根据用户的交互来改变元素的颜色、大小等。

2. **获取样式信息**：CSSOM还可以用于获取页面元素的样式信息。例如，你可以获取一个元素的颜色、边距等。

3. **优化渲染性能**：浏览器在创建渲染树时，会使用CSSOM和DOM。如果CSSOM可以快速地生成，那么页面的渲染速度就会更快。因此，优化CSSOM的构建速度是提高页面渲染性能的一种方法。
