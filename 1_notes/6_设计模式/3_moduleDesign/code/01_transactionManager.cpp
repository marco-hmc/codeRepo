...
* undo/redo和transaction的关系
  `undo` 和 `redo` 的实现通常依赖于某种形式的事务管理器（Transaction Manager），但具体实现方式可以有所不同。
  事务管理器的核心功能是管理一系列操作的执行和回滚，以确保系统的一致性和可靠性。
  `undo` 和 `redo` 功能可以看作是事务管理的一部分，特别是在编辑器、数据库和其他需要支持撤销和重做操作的应用程序中。
...
### 总结
  `undo` 和 `redo` 的实现通常依赖于某种形式的事务管理器，通过记录操作历史和状态变化来实现这些功能。
  事务管理器可以帮助管理 `undo` 和 `redo` 操作，确保系统的一致性和可靠性。
  通过使用事务管理器，可以方便地实现复杂的撤销和重做功能。
