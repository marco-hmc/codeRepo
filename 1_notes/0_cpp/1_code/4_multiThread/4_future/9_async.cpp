...
template <typename F> static auto asynchronous(F f) {
  return [f](auto... xs) {
    return [=]() { return async(launch::async, f, xs...); };
  };
}
...
template <typename F> static auto async_adapter(F f) {
  return [f](auto... xs) {
    return [=]() { return async(launch::async, fut_unwrap(f), xs()...); };
  };
}
