...
template <>
class ClassD<int>  // 1. template <>			2. ClassD<int>
{
    int b;
};
...
template <typename T>
class ClassD<T*>  // 1. template <typename T>	2. ClassD<T*>
{
    int d;
};

template <>
class ClassD<int*>  // 1. template <>			2. ClassD<T*>
{
    int e;
};
...
template <typename T, typename U>
void foo2(T const&, U const&);

#if WRONG_CODE_ENABLED
template <typename U>
void foo2<int, U>(int const&, U const&);
template <typename T, typename U>
void foo2<T, U>(int const&, U const&);
#endif

// Overloading - Looks like partial specification
template <typename U>
void foo2(int const&, U const&);
template <typename T, typename U>
void foo2(T const*, U const&);
...
UninferableT foo3(InferableT const&) {
    return UninferableT();
}
...
class is_class {
    ...
    // SFINAE
    template <typename U>
    static Accepted test(int U::*);
    template <typename U>
    static Rejected test(...);

    public:
    static const bool value =
        sizeof(test<T>(0)) == sizeof(Accepted);
};
...
class Convertible {
    ...
    public:
    static const bool value =
        sizeof(test(static_cast<Source*>(NULL))) ==
        sizeof(Accepted);
};
...
class is_base_of {
    public:
    static bool const value = is_class<Base>::value &&
...
class Fibonacci {
    ...
    static int const value =
        Fibonacci<Index - 1>::value + Fibonacci<Index - 2>::value;
};
...
void foo() { Do(static_cast<int*>(nullptr), int_<1>()); }

template <typename T, int i>
void DoAnotherWay(T* obj) {}
...
template <typename T>
void DoAnotherWay<T, 1>(T* obj) {}
template <typename T>
void DoAnotherWay<T, 2>(T* obj) {}
#endif
...
template <typename T0, typename T1 = Nil, typename T2 = Nil,
            typename T3 = Nil>
class make_vector_ {
