...
/*
  1. `std::scoped_lock`是什么，有什么用？
    `std::scoped_lock`是C++17引入的一个新特性，它是一个RAII风格的锁，用于管理一个或多个互斥锁的生命周期。
    当你创建一个`std::scoped_lock`对象时，它会自动以死锁避免的顺序锁定所有提供的互斥锁。
    当`std::scoped_lock`对象离开作用域时，它的析构函数会自动解锁所有的互斥锁。
    这样可以确保在发生异常或提前返回时，互斥锁总是被正确地解锁。

  2. `std::scoped_lock`和`std::mutex`的区别是什么？
    `std::mutex`是一个互斥锁对象，它提供了基本的锁定和解锁操作。
    你需要手动调用`std::mutex::lock`和`std::mutex::unlock`来锁定和解锁互斥锁。
    如果你忘记解锁互斥锁，或者在异常发生时没有正确地解锁互斥锁，那么可能会导致死锁。
    `std::scoped_lock`是一个锁的包装器，它在构造时自动锁定互斥锁，在析构时自动解锁互斥锁。
    这样可以确保互斥锁总是被正确地解锁，即使在发生异常或提前返回的情况下。
    此外，`std::scoped_lock`还可以同时管理多个互斥锁，以避免死锁。
