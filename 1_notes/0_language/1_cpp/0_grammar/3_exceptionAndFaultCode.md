...
catch的异常捕捉实现是基于类型比较的，使用类型比较而不是值比较的意义在于多态。另外catch的比较不是简单的类型相等比较，而是判断是不是在一个继承关系内，哪一个是最近的类型关系。在时间中，大部分异常都是派生自 `std::exception` 的，这个时候可以统一使用父类统一异常比较。
除了多态能力外，派生自 `std::exception` 的异常类会提供一个 `what()` 方法，用于描述异常类型，便于输出到日志中。
即能够提供更多的异常信息。

另外，实践中也有少部分直接使用枚举值作为异常的。因为catch的比较是类型比较，所以一般又会再接一个switch-case作为值比较。
