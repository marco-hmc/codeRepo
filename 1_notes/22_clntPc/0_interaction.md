...
2. **应用程序的事件队列**：
   ...
   - 在qt中这个事件队列被QApplication封装了
   - 事件循环不是单独一个线程的，但程序在阻塞的时候，这个事件队列还可以接受新的事件。（不知道为什么）
   - 程序进入未响应的时候就是因为主线程阻塞，事件队列无法再接受新的事件。
   - 测试程序如下：在qt中，每输入一个字符就显示这个字符，并停顿5s。一次性输入123456789字符，输入到7的时候就发生未响应。不识别之后的输入字符。
   而前面输入的1234567都会每隔5s显示出来。
...
> 键盘事件有哪些：
>   * 一般事件：键盘按下/释放/重复
可以简单理解为键盘只有按下/释放/重复三个事件，但对于修饰键则有特殊处理。如果检测到修饰键按下，事件的修饰位状态位就被修改为对应按键，修饰位状态位可以是多个按键。修饰位一般是修饰无关的。
qt则还是只会收到键盘事件，然后检查修饰键状态就知道是不是组合键了。

#### 1.6 操作系统是怎么处理选择集的？怎么处理选中对象的？怎么处理多选的？
> 鼠标事件有哪些：左键右键滚轮的点击和释放、鼠标移动、鼠标进入/离开窗口、滚轮移动、双击、拖拽开始/进行/结束。
从底层来说，即硬件、硬件驱动和鼠标事件的角度来看，不存在选择集的概念。选择集是一个抽象概念，需要客户端软件去实现和管理。
qt也是没有去抽象这个一个概念。
如果要处理多选的情况，在qt中一般是自己维护一个被选中对象的容器。
选择集的抽象层次是一个集合，选择集的子集是选择路径。
选择路径需要与选择对象关联起来，除此之外，选择对象一般还需要提供一个高亮能力以表示被选中。
